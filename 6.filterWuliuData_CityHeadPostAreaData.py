# 仅适用筛选"物流【即 快递】"订单的数据统计 且 地级市作为表头名称进行数据统计 且 各城市每月借书量(册次)的数据统计
...
def book_order_analysis(df):
    ...
    for col in df.columns:
        ...
        elif '城市' in col_lower and ('统计' in col_lower or 'match' in col_lower):
            col_mapping['city_stat'] = col
    ...
    # 如果城市统计列未自动识别，提示用户输入
    if 'city_stat' not in col_mapping:
        print("\n⚠️ 注意: 未自动识别到城市统计列，请手动指定")
        col_mapping['city_stat'] = input("请输入用于城市统计的列名: ").strip()
    
    order_col = col_mapping['order_type']
    qty_col = col_mapping['quantity']
    month_col = col_mapping['month']
    city_col = col_mapping['city_stat']
    ...
    # 验证城市统计列为数值型
    if not pd.api.types.is_numeric_dtype(df[city_col]):
        print(f"\n⚠️ 注意: '{city_col}'列为文本类型，尝试转换为数字...")
        df[city_col] = pd.to_numeric(df[city_col], errors='coerce')
        if df[city_col].isna().any():
            nan_count = df[city_col].isna().sum()
            print(f"❌ 转换失败，有 {nan_count} 行无法转换为数字")
            print("将使用0填充无效值")
            df[city_col] = df[city_col].fillna(0)
    ...
    for month in months:
        ...
        # ===========================================
        # 初始化城市计数字典（人次）
        city_counts = {city: 0 for city in GUANGDONG_CITIES}
        
        # 遍历借书订单，统计城市分布（人次）
        for _, row in borrow_data.iterrows():
            city_index = int(row[city_col])  # 转换为整数
            
            # 检查城市索引是否在有效范围内 (1-21)
            if 1 <= city_index <= len(GUANGDONG_CITIES):
                city_name = GUANGDONG_CITIES[city_index - 1]
        ...
        # 添加城市分布数据（人次）
        month_result.update(city_counts)
        ...
        # ===========================================
        # 初始化城市书籍字典（册次）
        city_books = {f"{city}借书量(册)": 0 for city in GUANGDONG_CITIES}
        ...
        for _, row in borrow_data.iterrows():
            ...
            if 1 <= city_index <= len(GUANGDONG_CITIES):
                city_name = GUANGDONG_CITIES[city_index - 1]
                # 累加该城市的书籍数量
                city_books[f"{city_name}借书量(册)"] += row[qty_col]
        ...
        book_result.update(city_books)
        city_book_results.append(book_result)
    
    # 创建结果DataFrame
    result_df = pd.DataFrame(results)
    
    # 创建城市借书册数DataFrame
    city_book_df = pd.DataFrame(city_book_results)
    
    # 添加总计行
    if not result_df.empty:
        total_row = {'月份': '总计'}
        # 添加基础列总计
        base_cols = ['借书量(册)', '还书量(册)', '借书人次', '还书人次', '净借书量(册)']
        ...
        for city in GUANGDONG_CITIES:
            if city in result_df.columns:
                total_row[city] = result_df[city].sum()
    ...
    # 为城市借书册数表添加总计行
    if not city_book_df.empty:
        book_total_row = {'月份': '总计', '总借书量(册)': city_book_df['总借书量(册)'].sum()}
        # 添加城市借书册数总计
        for col in city_book_df.columns:
            if col.endswith('借书量(册)') and col != '总借书量(册)':
                book_total_row[col] = city_book_df[col].sum()
        
        # 创建总计行DataFrame并合并
        book_total_df = pd.DataFrame([book_total_row])
        city_book_df = pd.concat([city_book_df, book_total_df], ignore_index=True)
    ...
    # 只显示部分关键列和城市列，避免控制台输出过长
    key_cols = ['月份', '借书量(册)', '还书量(册)', '借书人次', '还书人次', '净借书量(册)']
    display_cols = key_cols + GUANGDONG_CITIES[:5]  # 显示前5个城市列
    
    # 只显示前几列，避免控制台输出过长
    print(result_df[display_cols].to_string(index=False))
    
    # 提示用户查看完整数据
    print(f"\n💡 提示: 完整数据包含 {len(result_df.columns)} 列，将在导出文件中显示所有城市")
    ...
    if not result_df.empty:
        ...
        if len(months) > 1:
            # 找出借书最多的城市（人次）
            city_sums = result_df.iloc[:-1][GUANGDONG_CITIES].sum()  # 排除总计行
            max_city = city_sums.idxmax()
            max_city_value = city_sums.max()
            
            # 找出借书册数最多的城市
            book_cols = [f"{city}借书量(册)" for city in GUANGDONG_CITIES]
            book_sums = city_book_df.iloc[:-1][book_cols].sum()  # 排除总计行
            max_book_city = book_sums.idxmax().replace('借书量(册)', '')
            max_book_value = book_sums.max()
            
            print(f"\n🌆 城市借书量分析:")
            print(f"• 借书人次最多的城市: {max_city} ({max_city_value}人次)")
            print(f"• 借书册数最多的城市: {max_book_city} ({max_book_value}册)")
    else:
        print("⚠️ 没有统计数据")
    ...
    if export == 'y':
        ...
        try:
            ...
            with pd.ExcelWriter(export_path, engine='openpyxl') as writer:
                # 写入统计结果（人次）
                result_df.to_excel(writer, sheet_name='月度统计（人次）', index=False)
                
                # 写入城市借书册数统计
                city_book_df.to_excel(writer, sheet_name='城市借书册数', index=False)
                ...
                try:
                    if not result_df.empty and len(result_df) > 1:  # 需要多于一行数据（不包括总计）
                        ...
                        # 在城市借书册数sheet中添加图表
                        ws_city = workbook['城市借书册数']
                        
                        # 创建城市借书册数图表
                        city_chart = BarChart()
                        city_chart.type = "col"
                        city_chart.style = 12
                        city_chart.title = "城市借书册数统计"
                        city_chart.y_axis.title = '册数'
                        city_chart.x_axis.title = '城市'
                        
                        # 获取城市列位置
                        city_cols = [col for col in city_book_df.columns if col.endswith('借书量(册)') and col != '总借书量(册)']
                        num_cities = len(city_cols)
                        
                        # 添加城市数据（只显示前10个城市）
                        city_data = Reference(ws_city, min_col=3, min_row=1, max_col=min(13, 3+10), max_row=len(city_book_df)-1)
                        city_categories = Reference(ws_city, min_col=3, min_row=2, max_col=min(13, 3+10), max_row=2)  # 使用标题行作为分类
                        
                        city_chart.add_data(city_data, titles_from_data=True)
                        city_chart.set_categories(city_categories)
                        
                        # 放置图表
                        ws_city.add_chart(city_chart, "B10")
            ...
            print("包含以下工作表:")
            print("- 月度统计（人次）: 按月份统计的借还书情况（人次分布）")
            print("- 城市借书册数: 各城市借书册数详细统计")
            if include_raw == 'y':
                print("- 原始数据: 用于分析的原始数据")
        except Exception as e:
            print(f"\n❌ 导出失败: {str(e)}")
