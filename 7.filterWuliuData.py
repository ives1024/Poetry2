# 仅适用筛选"物流【即 快递】"订单的数据统计，邮寄区域分布不包含各城市的借书册次统计（各城市非表头呈现）
...
def book_order_analysis(df):
    ...
    for month in months:
        ...
        # 生成邮寄区域分布字符串
        distribution_str = ""
        for i, city in enumerate(GUANGDONG_CITIES):
            count = city_counts[city]
            # 只显示有订单的城市，或者显示所有城市（包括0）
            distribution_str += f"{city}({count})"
            if i < len(GUANGDONG_CITIES) - 1:
                distribution_str += "、"
